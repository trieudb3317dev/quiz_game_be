...
import { GenderType, User } from '../users/user.entity';
import { Repository } from 'typeorm';
import * as bcrypt from 'bcrypt';
import { RegisterDto, ResponseDto, UpdateUserDto } from './auth.dto';
...
export class AuthService {
  ...
  async me(userId: number): Promise<ResponseDto> {
    ...
    const responseUser: ResponseDto = {
      id: user.id,
  ...
  async update(
    ...
    try {
      ...
      this.logger.error('Error during user update', error.stack);
      throw new HttpException(
  ...
  async requestOwnerRole(userId: number): Promise<{ message: string }> {
    // Implement request owner role logic here
    this.logger.log(`User with ID ${userId} is requesting owner role`);
    const user = await this.userRepository.findOne({ where: { id: userId } });
    if (!user) {
      throw new HttpException('User not found', HttpStatus.NOT_FOUND);
    }
    await this.userRepository.update(userId, { is_owner: true });
    this.logger.log(`User with ID ${userId} granted owner role`);
    return { message: 'Owner role requested successfully' };
  }
  ...
  private generateJwtToken(user: User, response: Response): string {
  ...
  private generateRefreshToken(user: User, response: Response): string {
