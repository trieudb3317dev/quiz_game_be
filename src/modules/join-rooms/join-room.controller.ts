...
export class JoinRoomController {
    ...
    @ApiOperation({ summary: 'List joiners of a room (supports search, sort, pagination)' })
    ...
    @ApiOperation({ summary: 'Get a joiner record for a specific user in a room' })
    ...
    @HttpCode(HttpStatus.OK)
    async getJoiner(@Param('roomId') roomId: number, @Param('userId') userId: number) {
